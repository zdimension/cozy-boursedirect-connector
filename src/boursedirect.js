      ...
      if (newAsset) {
        ...
        if (data[i][0] == 'Total') {
          rtnData['assets'][assetCount][2] = makeFloat(
            data[i][originalColumns['pmvalues']]
          )
        }
      }
    }
...
class BourseDirectApi {
  ...
  async fetch(url, method = 'GET', body = null) {
    log(
      'info',
      `req on ${url}: ${method} ${body} ${JSON.stringify(
  ...
  async getData() {
    ...
    const data = await this.fetch(
      'streaming/compteTempsReelCK.php?stream=0&nc=1'
    )
    const parsed = parseData(
      data.substring("message='".length, data.length - 1)
    )
    ...
    for (const row of rows) {
      ...
      const opId = `${id}-${y}-${m}-${d}-${label.replace(
        /[ .]/g,
        '-'
      )}-${Math.floor(amount)}`
